...
CREATE TABLE tipoMensaje(
	idTipoMensaje int not null auto_increment primary key,
	nombreTipoMensaje varchar(35) not null
);
...
CREATE TABLE mensaje(
	...
	contenido text null,
	archivoAdjunto varchar(200) null,
	leido bool not null,
	FOREIGN KEY (idTipoMensaje) REFERENCES tipoMensaje (idTipoMensaje) ON UPDATE CASCADE ON DELETE CASCADE,
	FOREIGN KEY (idUsuario_Remitente) REFERENCES usuario (idUsuario) ON UPDATE CASCADE ON DELETE CASCADE,
	FOREIGN KEY (idUsuario_Destinatario) REFERENCES usuario (idUsuario) ON UPDATE CASCADE ON DELETE CASCADE	
);

CREATE TABLE documentosPersonal(
	idDocumentosPersonal int not null auto_increment primary key,
	idPersonal int not null,
	nombreDocumento varchar(150) not null,
	rutaDocumento varchar(350) not null,
	FOREIGN KEY (idPersonal) REFERENCES personal (idPersonal) ON UPDATE CASCADE ON DELETE CASCADE	
);
